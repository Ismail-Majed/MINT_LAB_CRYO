...
def calculatereceiverGBK():
    ...
    for index,row in enumerate(noiseParamColdCSV.iterrows()):
       complex_num = cmath.rect(10**(row[1][0]/10),pi*(row[1][1]/180))
    ...
    Frec=10*log10((ENRlin-Y_factor*((Tcold/T0)-1))/(Y_factor-1))
    kBG=Nhot_linear/(Thot+(Frec-1)*T0)
    return kBG
